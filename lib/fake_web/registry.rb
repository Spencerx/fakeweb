module FakeWeb
  class Registry #:nodoc:
    ...
    attr_accessor :uri_map, :pass_through_uri_map
    ...
    def clean_registry
      ...
      self.pass_through_uri_map = Hash.new { |hash, key| hash[key] = {} }
    end
    ...
    def register_pass_through_uri(method, uri)
      pass_through_uri_map[normalize_uri(uri)][method] = true
    end

    def registered_pass_through_uri?(method, uri)
      uri = normalize_uri(uri)

      uri_map_matches(pass_through_uri_map, method, uri, URI) ||
      uri_map_matches(pass_through_uri_map, :any,   uri, URI) ||
      uri_map_matches(pass_through_uri_map, method, uri, Regexp) ||
      uri_map_matches(pass_through_uri_map, :any,   uri, Regexp)
    end
